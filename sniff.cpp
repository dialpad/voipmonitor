...
//			}
		...
		*z = zchr;
		if(codec || payload) {
			...
			syslog(LOG_DEBUG,"Inside get_rtpmap_from_sdp(), codec = %d",codec);
...
int get_ip_port_from_sdp(Call *call, packet_s_process *packetS, char *sdp_text, size_t sdp_text_len,
	...
	syslog(LOG_DEBUG,"Starting get_ip_port_from_sdp() function");
	unsigned long l;
	char *s;
...
//		    syslog(LOG_DEBUG,"Logging application");
			...
			syslog(LOG_DEBUG,"After gettag media_type= %d",(int)media_type);
			vmPort port;
...
//				syslog(LOG_DEBUG,"sdp_media_start: %c, sdp_media_type= %s, sdp_media_port = %s",sdp_media_start[sdp_media_start_count],to_string(sdp_media_type[sdp_media_start_count]),to_string(sdp_media_port[sdp_media_start_count]));
				...
				syslog(LOG_DEBUG,"First break");
				break;
			}
		} else {
			syslog(LOG_DEBUG,"Second break");
			break;
		}
	}
...
//			continue;
		...
		if(sdp_media_type[sdp_media_i] != sdp_media_type_application) {
			syslog(LOG_DEBUG,"getting rtpmap");
	...
	syslog(LOG_DEBUG,"sdp_media_counter=%u",sdp_media_counter);
				...
				if(ok_ip_port) {
					...
					if(opt_rtp_firstleg == 0 || 
					   (opt_rtp_firstleg &&
						...
						syslog(LOG_DEBUG,"sdp_media_data_item->rtpmap.payload= %d , sdp_media_data_item->rtpmap.codec = %d", sdp_media_data_item->rtpmap.payload, sdp_media_data_item->rtpmap.codec);
